...
class IonBeamReconstructor:
    ...
    def export_low_res_beam_csv(self, output_path, size=30, resolution=1):
        ...
        x_efficiency = np.trapz(self.x_data_raw, self.x_pos_raw) / np.trapz(time_integrated_x, self.x_pos_raw)
        
        # Y方向: 沿X扫描 (时间因子 = X扫描长度 / 速度)
        time_integrated_y = pred_y_rel * self.y_time_factor
        y_efficiency = np.trapz(self.y_data_raw, self.y_pos_raw) / np.trapz(time_integrated_y, self.y_pos_raw)
    ...
    def validate_etch_profiles(self):
        ...
        for i in range(len(grid_points)):
            ...
            simulated_etch_x[i] = np.trapz(etch_per_point, grid_points)
        ...
        rel_error_x = np.abs((interp_x - original_x_norm) / original_x_norm)
        avg_rel_error_x = np.mean(rel_error_x) * 100
        ...
        for j in range(len(grid_points)):
            ...
            simulated_etch_y[j] = np.trapz(etch_per_point, grid_points)
        ...
        rel_error_y = np.abs((interp_y - original_y_norm) / original_y_norm)
        avg_rel_error_y = np.mean(rel_error_y) * 100
