...
class IonBeamReconstructor:
    def __init__(self, x_profile_path, y_profile_path, model_type='asymmetric'):
        """初始化束流重构系统"""
        ...
        # 添加模型类型参数
        self.model_type = model_type  # 'symmetric' 或 'asymmetric'
    ...
    def asymmetric_gaussian_model(self, params):
        """非对称高斯束流模型，支持偏斜分布"""
        x0, y0 = params[0], params[1]
        sig_x, sig_y, amp, skew_x, skew_y, theta = params[2], params[3], params[4], params[5], params[6], params[7]
        ...
        coords = np.stack([self.xx.ravel() - x0, self.yy.ravel() - y0])
        rotated_coords = np.dot(rotation_matrix, coords)
        x_rot, y_rot = rotated_coords[0].reshape(self.xx.shape), rotated_coords[1].reshape(self.yy.shape)
        
        # 非对称偏斜项
        # 使用双曲正切实现平滑过渡的非对称性
        skew_term_x = 1 + np.tanh(skew_x * x_rot / sig_x)
        skew_term_y = 1 + np.tanh(skew_y * y_rot / sig_y)
        
        # 计算束流分布
        g = amp * np.exp(-0.5 * (x_rot**2 / sig_x**2 + y_rot**2 / sig_y**2)) * skew_term_x * skew_term_y
    ...
    def get_beam_model(self, params):
        ...
        if self.model_type == 'asymmetric':
            return self.asymmetric_gaussian_model(params)
        else:
    ...
    def model_mismatch(self, params):
        ...
        # 根据模型类型获取角度值
        if self.model_type == 'asymmetric':
            current_angle = params[7]  # 非对称模型的角度在索引7
        else:
            current_angle = params[8]  # 对称模型的角度在索引8
    ...
    def reconstruct_beam(self):
        ...
        # 设置边界和初始猜测        
        if self.model_type == 'asymmetric':
            print("使用非对称高斯模型 (8个参数)")
            # 非对称模型的参数边界
            bounds = [
                (-5, 5), (-5, 5),          # x0, y0
                (0.1, 10), (0.1, 10),       # sig_x, sig_y
                (0.01, 2),                  # amp
                (-1.5, 1.5), (-1.5, 1.5),   # skew_x, skew_y (偏斜参数)
                (-60, 60)                   # theta
            ]
            
            initial_guess = [
                0.0, 0.0,                   # x0, y0
                np.ptp(self.x_pos)/4,        # sig_x
                np.ptp(self.y_pos)/4,        # sig_y
                1.0,                         # amp
                0.0, 0.0,                    # 初始偏斜设为0
                0.0                          # theta
            ]
        ...
        try:
            ...
            # 根据模型类型获取角度值
            angle_idx = 7 if self.model_type == 'asymmetric' else 8
            self.optimization_stages.append(f"全局优化结束 θ={initial_guess[angle_idx]:.1f}°")
        ...
        if len(self.angle_history) > 0:
            last_angle = initial_guess[angle_idx]
        ...
        if self.model_type == 'asymmetric':
            ...
            phase2_bounds = [
                ...
                (max(-2.0, initial_guess[5]-0.5), min(2.0, initial_guess[5]+0.5)),
                (max(-2.0, initial_guess[6]-0.5), min(2.0, initial_guess[6]+0.5)),
                angle_bounds
            ]
        else:
            # 对称模型的边界 (9个参数)
        ...
        # 在模型计算中使用get_beam_model
        self.beam_model = self.get_beam_model(result.x)
        
        # 根据模型类型获取最终角度值
        angle_idx = 7 if self.model_type == 'asymmetric' else 8
        self.optimization_stages.append(f"优化完成 θ={result.x[angle_idx]:.1f}°")
        ...
        if len(self.angle_history) > 0:
            print(f"\n误差从 {self.error_history[0]:.4f} 减少到 {self.error_history[-1]:.4f}")
    ...
    def export_low_res_beam_csv(self, output_path, size=30, resolution=1):
        ...
        # 计算当前最优参数下的束流分布（未归一化）
        beam_profile = self.get_beam_model(self.optimal_params)
...
if __name__ == "__main__":
    ...
    reconstructor = IonBeamReconstructor(
        ...
        model_type='asymmetric'  # 明确指定模型类型
    )
