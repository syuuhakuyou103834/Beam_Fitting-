...
class BeamEfficiencyOptimizer:
    ...
    def enforce_derivative_constraints_global(self, beam_matrix, center_i, center_j):
        """确保整个平面上满足导数特性，只允许中心点处二阶导为零"""
        try:
            self.log("检查并执行全局导数约束")
            center_pos = (self.grid[center_i], self.grid[center_j])
            modified_points = 0
            
            # 在多个角度方向上检查导数
            angles = np.linspace(0, 2*np.pi, 24, endpoint=False)  # 24个方向
            
            for angle in angles:
                dx, dy = np.cos(angle), np.sin(angle)
                distances = np.arange(0, self.grid_bound + 0.5, 0.5)
                ...
                # 筛选有效索引（整个区域）
                valid_mask = (x_indices >= 0) & (x_indices < 31) & \
                            (y_indices >= 0) & (y_indices < 31)
                
                x_path = x_indices[valid_mask]
                y_path = y_indices[valid_mask]
                dist_path = distances[valid_mask]
                
                if len(x_path) < 3:
                    continue
                    
                # 获取射线上的值
                values = beam_matrix[x_path, y_path]
                
                # 计算一阶和二阶导数
                grad = np.gradient(values, dist_path)
                grad2 = np.gradient(grad, dist_path)
                
                # 检查并修正一阶导数（只在中心点外必须为负）
                for i in range(1, len(grad)):
                    # 如果接近中心点（中心点附近允许导数为正）
                    distance_to_center = dist_path[i]
                    if distance_to_center < 1.0:  # 中心点附近区域
                        continue
                        
                    # 在中心点外，必须严格递减
                    if grad[i] >= 0:  # 一阶导数非负，不符合要求
                        # 修改值使其递减
                        prev_val = beam_matrix[x_path[i-1], y_path[i-1]]
                        new_val = prev_val * 0.95
                        beam_matrix[x_path[i], y_path[i]] = new_val
                        modified_points += 1
                
                # 检查二阶导数符号变化（只修复远离中心点的突然变化）
                if len(dist_path) > 5:
                    for i in range(2, len(grad2)-2):
                        # 跳过中心点附近
                        if dist_path[i] < self.center_threshold:
                            continue
                            
                        # 检测突然的曲率变化
                        if abs(grad2[i]) > 0.01 and np.sign(grad2[i-1]) != np.sign(grad2[i+1]):
                            # 用周围点进行平滑
                            new_val = (beam_matrix[x_path[i-1], y_path[i-1]] + 
                                    beam_matrix[x_path[i], y_path[i]] * 2 +
                                    beam_matrix[x_path[i+1], y_path[i+1]]) / 4
                            beam_matrix[x_path[i], y_path[i]] = new_val
                            modified_points += 1
            
            if modified_points > 0:
                self.log(f"全局导数约束: 调整了 {modified_points} 个点以确保平滑性")
        except Exception as e:
            self.log(f"全局导数约束执行失败: {str(e)}")
        
        return beam_matrix

    
    #def enforce_derivative_constraints(self, beam_matrix):
        """确保非中心区域满足导数特性，要求一阶导数恒为负，二阶导数不变号"""
        try:
            self.log("检查并强制执行非中心区域的导数约束")
            ...
            # 创建非中心区域掩膜
            non_center_mask = self.edge_mask | self.outer_mask
            ...
            for angle in angles:
                ...
                valid_mask = (x_indices >= 0) & (x_indices < 31) & \
                            (y_indices >= 0) & (y_indices < 31) & \
                            (np.sqrt((self.grid[x_indices]-center_pos[0])**2 + 
                                    (self.grid[y_indices]-center_pos[1])**2) > self.center_threshold)
                ...
                for i in range(1, len(grad)):
                    if grad[i] >= 0:  # 一阶导数非负，不符合要求
                        # 修改值使其递减
                        prev_val = beam_matrix[x_path[i-1], y_path[i-1]]
                        new_val = prev_val * 0.95 if prev_val > 0 else 0
                        beam_matrix[x_path[i], y_path[i]] = min(beam_matrix[x_path[i], y_path[i]], new_val)
                        modified_points += 1
                ...
                for idx in sign_changes:
                    if idx > 0 and idx < len(values) - 1:
                        ...
                        before = values[idx-1]
                        after = values[idx+1]
                        new_val = (before + after) * 0.5
                        beam_matrix[x_path[idx], y_path[idx]] = new_val
                        modified_points += 1
            
            if modified_points > 0:
                self.log(f"导数约束: 调整了 {modified_points} 个点以确保平滑性")
        except Exception as e:
            self.log(f"导数约束执行失败: {str(e)}")
    ...
    def run_optimization(self, max_iterations=100, target_rel_error=10.0):
       ...
       # ================== 新增第四阶段：智能平滑优化 ==================
        ...
        # 1. 标记需要保留的点
        self.log("标记保留点：峰值点、匹配良好的点和边缘零值点")
        preserve_mask = np.zeros_like(pre_smoothing_matrix, dtype=bool)

        # 1.1 中心区域的极大值顶点（峰值点）
        max_idx = np.unravel_index(np.argmax(pre_smoothing_matrix), pre_smoothing_matrix.shape)
        preserve_mask[max_idx] = True
        self.log(f"标记峰值点: ({self.grid[max_idx[0]]}, {self.grid[max_idx[1]]})")

        # 1.2 模拟与实验数据匹配良好的点（差值 ≤ 0.04）
        current_sim_x = self.simulate_etching(pre_smoothing_matrix, "x")
        current_sim_y = self.simulate_etching(pre_smoothing_matrix, "y")

        # X方向匹配良好的点
        exp_x_data = self.beam_traced_y_axis
        exp_x_val_norm = exp_x_data[:, 1] / np.max(exp_x_data[:, 1])
        sim_x_interp = np.interp(exp_x_data[:, 0], self.grid, current_sim_x)
        x_diff = np.abs(sim_x_interp - exp_x_val_norm)
        good_x_points = np.unique(
            [np.argmin(np.abs(self.grid - pos)) for pos in exp_x_data[x_diff <= 0.049, 0]]
        )
        for i in good_x_points:
            j_range = np.where((np.abs(self.grid) < 0.5) & (self.grid.size == pre_smoothing_matrix.shape[0]))
            if j_range[0].size > 0:
                j = j_range[0][0]
                preserve_mask[i, j] = True
        self.log(f"标记X方向匹配点: {len(good_x_points)}个点")

        # Y方向匹配良好的点
        exp_y_data = self.beam_traced_x_axis
        exp_y_val_norm = exp_y_data[:, 1] / np.max(exp_y_data[:, 1])
        sim_y_interp = np.interp(exp_y_data[:, 0], self.grid, current_sim_y)
        y_diff = np.abs(sim_y_interp - exp_y_val_norm)
        good_y_points = np.unique(
            [np.argmin(np.abs(self.grid - pos)) for pos in exp_y_data[y_diff <= 0.049, 0]]
        )
        for j in good_y_points:
            i_range = np.where((np.abs(self.grid) < 0.5) & (self.grid.size == pre_smoothing_matrix.shape[0]))
            if i_range[0].size > 0:
                i = i_range[0][0]
                preserve_mask[i, j] = True
        self.log(f"标记Y方向匹配点: {len(good_y_points)}个点")

        # 1.3 边缘的零值点（值 < 0.001）
        zero_mask = pre_smoothing_matrix < 0.001
        preserve_mask = preserve_mask | zero_mask
        self.log(f"标记零值点: {np.sum(zero_mask)}个点")

        # 2. 平滑整个区域但保留标记点
        self.log("执行智能平滑：保留关键点的高斯滤波")
        from scipy.ndimage import gaussian_filter

        # 保存需要保留的值
        preserved_values = pre_smoothing_matrix[preserve_mask]

        # 对整个矩阵进行平滑
        smoothed_matrix = gaussian_filter(pre_smoothing_matrix, sigma=1.5)

        # 恢复保留点的值
        smoothed_matrix[preserve_mask] = preserved_values

        # 3. 应用径向约束，然后再次恢复保留点
        smoothed_matrix = self.enforce_radial_constraints(smoothed_matrix)
        smoothed_matrix[preserve_mask] = preserved_values  # 再次确保保留点的值不变

        self.log("智能平滑完成，保留点恢复")
        ...
        sim_y = self.simulate_etching(current_matrix, "y")
        abs_error, rel_err_x, rel_err_y = self.calculate_error(sim_x, sim_y)
        self.log(f"平滑后误差: 绝对={abs_error:.4f}, X向={rel_err_x:.1f}%, Y向={rel_err_y:.1f}%")

        # 记录平滑后的状态
        smooth_iter_index = len(self.history["iteration"])
        self.history["iteration"].append(smooth_iter_index)
        self.history["abs_error"].append(abs_error)
        self.history["rel_err_x"].append(rel_err_x)
        self.history["rel_err_y"].append(rel_err_y)
        self.stage_ends.append(smooth_iter_index)

        # 保存最佳模拟轮廓
        best_sim_smoothing = (sim_x, sim_y)

        # 第四阶段迭代优化（微调）
        smoothing_iterations = 10  # 较少的迭代次数，主要是微调

        for iteration in range(1, smoothing_iterations + 1):
            # 动态调整变异幅度（非常小）
            magnitude = self.calculate_magnitude(iteration, smoothing_iterations) * 0.3
            
            self.log(f"迭代 {iteration}: 第4阶段(平滑微调), 变异幅度={magnitude:.3f}")
            ...
            for _ in range(self.max_mutations):  
                ...
                # 强制保持保留点不变
                candidate[preserve_mask] = preserved_values
                candidates.append(candidate)
            ...
            if best_candidate is not candidates[0]:
                improvement = self.history["abs_error"][-1] - best_abs_error
                self.log(f"平滑微调: Δ={improvement:.5f}, 新绝对误差={best_abs_error:.4f}")
                stagnation_count = 0
                self.log(f"相对误差: X向={best_rel_errs[0]:.1f}%, Y向={best_rel_errs[1]:.1f}%")
                
                # 更新历史记录
                iter_index = smooth_iter_index + iteration
                self.history["iteration"].append(iter_index)
                self.history["abs_error"].append(best_abs_error)
                self.history["rel_err_x"].append(best_rel_errs[0])
                self.history["rel_err_y"].append(best_rel_errs[1])
                ...
                stagnation_count += 1
                self.log(f"无平滑微调改进，维持误差: {best_abs_error:.4f}")
    ...
    def visualize_results(self):
        ...
        try:
            ...
            if len(self.history["iteration"]) > 1:
                ...
                if len(self.stage_ends) >= 5:  # 显示第四阶段结束点
                    ax7.axvline(
                        self.stage_ends[4], 
                        color='c',  # 青色表示平滑微调结束
                        linestyle='-', 
