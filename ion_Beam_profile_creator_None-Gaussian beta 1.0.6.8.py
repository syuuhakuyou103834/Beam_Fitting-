...
class BeamEfficiencyOptimizer:
    ...
    def create_optimization_mask(self):
        ...
        self.log(f"优化区域包含 {np.sum(self.optimization_mask)} 个点 (半径 ≤ {self.opt_radius}mm)，不包括最外圈零值区")
    ...
    def simulate_etching(self, beam_matrix_normalized, direction):
        """
        修改后的模拟刻蚀轮廓方法
        使用实际束流值（不归一化）进行卷积，然后乘以1/30（移动速度30mm/s）
        """
        # 使用归一化的束流乘以最大值得到实际束流值
        ...
        if direction == "x":  # 束沿Y轴移动 -> 测量沿X方向的轮廓
            for j in range(len(self.grid)):
                ...
                profile[j] = integral_result * (1/self.scan_velocity)
            ...
            for i in range(len(self.grid)):
                ...
                # 计算积分结果后乘以1/30
                integral_result = trapezoid(etch_rates, dx=dx)
                profile[i] = integral_result * (1/self.scan_velocity)
    ...
    def run_optimization(self):
        ...
        # 绘制等高线凸性验证图
        self.plot_convexity_verification(optimized_beam_full)
