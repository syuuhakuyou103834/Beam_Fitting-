...
def down_sample_beam(beam_matrix, src_resolution=0.1, target_resolution=1.0):
    ...
    参数:
        beam_matrix (ndarray): 源束流分布 (高分辨率)
        src_resolution (float): 源网格分辨率 (mm)
        target_resolution (float): 目标网格分辨率 (mm)
    
    返回:
        ndarray: 降采样后的束流分布
    """
    # 计算网格点范围 (对称)
    grid_bound = GRID_BOUND  # 使用原来的边界设定
...
class DirectionalBeamOptimizer:
    ...
    def calculate_error(self, sim_x, sim_y):
        ...
        # 综合误差 (X方向权重60%)
        combined_error = 0.5 * rmse_x + 0.5 * rmse_y
    ...
    def run_optimization(self, iterations=90):
        ...
        np.savetxt("directional_optimized_beam_highres_0.1mm.csv", output_beam_highres, delimiter=",")
        
        # 保存低精度1mm版本
        output_beam_lowres = down_sample_beam(output_beam_highres, HIGH_PRECISION, 1.0)
        np.savetxt("directional_optimized_beam_lowres_1mm.csv", output_beam_lowres, delimiter=",")
        ...
        with open("decay_parameters.txt", "w") as f:
            ...
            f.write(f"# 输出格式:\n")
            f.write(f"#   - High-res (0.1mm): directional_optimized_beam_highres_0.1mm.csv\n")
            f.write(f"#   - Low-res  (1mm)  : directional_optimized_beam_lowres_1mm.csv\n")
        ...
        self.log(f"已保存高精度 (0.1mm) 束流分布: directional_optimized_beam_highres_0.1mm.csv")
        self.log(f"已保存低精度 (1mm)  束流分布: directional_optimized_beam_lowres_1mm.csv")
    ...
    def visualize_results(self, init_sim_x, init_sim_y, final_sim_x, final_sim_y, errors, unimodal, highres_beam):
        """创建可视化报告"""
        try:
    ...
    def finalize(self):
        """结束优化并关闭日志"""
        self.log("优化完成! 结果已保存")
        self.log("  - directional_optimized_beam_highres_0.1mm.csv (优化后的束流分布-高精度0.1mm)")
        self.log("  - directional_optimized_beam_lowres_1mm.csv (优化后的束流分布-低精度1mm)")
...
def main():
    ...
    try:
        ...
        print("结果文件:")
        print("  - directional_optimized_beam_highres_0.1mm.csv (优化后束流分布-高精度0.1mm)")
        print("  - directional_optimized_beam_lowres_1mm.csv (优化后束流分布-低精度1mm)")
