...
MIN_RADIAL_SLOPE = 0.008  # 最小径向斜率约束
...
class DirectionalBeamOptimizer:
    ...
    def enforce_strict_unimodality(self, beam_matrix):
        ...
        for r_bin in radial_bins[1:]:
            ...
            inner_values = beam_new[inner_mask]
            ...
            for idx in range(len(indices[0])):
                ...
                # 确定参考值（最近内点的最大值乘以方向衰减因子）
                ref_val = np.max(inner_values) * decay_factor
                
                # 应用点约束
                if beam_new[i, j] > ref_val:
                    beam_new[i, j] = ref_val
                
                # 最小径向梯度约束
                min_val = np.max(inner_values) - MIN_RADIAL_SLOPE * RING_STEP
                if beam_new[i, j] < min_val:
                    beam_new[i, j] = min_val
        
        # 确保中心点仍为最大值
        beam_new[self.center_idx, self.center_idx] = np.max(beam_new)
        
        # 高斯平滑 (0.1mm半径)
        beam_new = gaussian_filter(beam_new, sigma=0.5)
    ...
    def validate_unimodality_profile(self, profile):
        ...
        for peak_idx in peaks:
            ...
            if abs(peak_pos - center_pos) > 0.5:
    ...
    def run_optimization(self, iterations=90):
        ...
        for i in range(iterations):
            ...
            # 突变因子（随迭代次数减少）
            mutation_factor = 0.18 * (1 - i/iterations)
            
            # 生成变异候选
            candidate = self.optimized_beam.copy()
            
            # 在优化区域内变异
            r_vals = self.r_matrix.flatten()
            mutation_ratio = 0.45 * (1 - i/iterations)  # 变异比例递减
            
            # 选择要变异的点 (距离依赖概率)
            mutation_mask = (r_vals < self.opt_radius) & (np.random.random(r_vals.shape) < mutation_ratio)
            mutate_values = mutation_factor * (np.random.random(np.sum(mutation_mask)) - 0.5)
            
            # 应用变异
            candidate_flat = candidate.flatten()
            candidate_flat[mutation_mask] += mutate_values
            candidate = candidate_flat.reshape(candidate.shape)
        ...
        # 保存输出
        output_beam = self.optimized_beam * self.max_val
    ...
    def visualize_results(self, init_sim_x, init_sim_y, final_sim_x, final_sim_y, errors, unimodal):
        ...
        try:
            ...
            # 标记方向差异
            x_diff = np.max(np.abs(init_sim_y - final_sim_y))
            ax6.text(0.1, 0.9, f"Y方向最大改进: {x_diff:.4f}", 
