...
def save_beamprofile_with_diffs(beam_profile, x_profile, y_profile, iteration, folder='iteration'):
    ...
    # 保存到文件
    filename = os.path.join(folder, f"beamprofile_iteration_{iteration}.csv")
    df.to_csv(filename, index=False)
    print(f"已保存迭代{iteration}的beamprofile至: {filename}")
...
def reconstruct_beam_profile():
    ...
    # 保存平移后的截面数据（新添加功能）
    save_shifted_profile(x_positions, x_profile, 
                         'translated_x_crosssection.csv')
    
    save_shifted_profile(y_positions, y_profile_raw, 
                         'translated_y_crosssection.csv')
    
    # 保存组合的初始截面数据
    save_initial_profiles(
        x_positions, x_profile, 
        y_positions - y_offset, y_profile_raw  # 应用偏移后的位置
    )
    ...
    while np.sum(~adjusted) > 0:  # 当还有未处理行时继续
        ...
        for row in unadjusted_rows:
            # 只考虑绝对值大于1e-6的差异
            if np.abs(diff_y_vec[row]) > 1e-3 and diff_y_vec[row] > max_val:
                max_val = diff_y_vec[row]
                selected_row = row
                edge_distance = np.abs(row - 15)
            # 当出现相同最大值时，选择边缘行
            elif np.abs(diff_y_vec[row] - max_val) < 1e-6 and np.abs(diff_y_vec[row]) > 1e-3:
                current_edge_distance = np.abs(row - 15)
                # 优先选择更靠近边缘的行
                if current_edge_distance > edge_distance:
                    max_val = diff_y_vec[row]
                    selected_row = row
                    edge_distance = current_edge_distance
                # 当两个边缘距离相同时，选择索引值小的（y坐标大的）
                elif current_edge_distance == edge_distance and row < selected_row:
                    selected_row = row
        
        # 检查是否找到有效行
        if selected_row == -1 or np.abs(max_val) < 1e-3:
            print("没有需要调整的行 - 跳过迭代")
            break
            
        max_diff_val = diff_y_vec[selected_row]
        y_position = 15 - selected_row
        print(f"调整行: y={y_position}mm (第{selected_row+1}行), 差异值={max_diff_val:.6f}")
