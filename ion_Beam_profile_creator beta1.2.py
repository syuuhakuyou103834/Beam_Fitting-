...
class IonBeamReconstructor:
    ...
    def export_low_res_beam_csv(self, output_path, size=30, resolution=1):
        ...
        # 在低分辨率网格上插值获取刻蚀能力值
        def interpolate_beam(x_val, y_val):
            """在高分辨率模型上进行双线性插值"""
            # 找到最近的四个网格点
            idx_x = np.searchsorted(self.xx[0, :], x_val)
            idx_y = np.searchsorted(self.yy[:, 0], y_val)
            
            # 防止索引越界
            idx_x = max(0, min(idx_x, self.xx.shape[1]-2))
            idx_y = max(0, min(idx_y, self.yy.shape[0]-2))
            
            # 获取四个点坐标
            x0, x1 = self.xx[0, idx_x], self.xx[0, idx_x+1]
            y0, y1 = self.yy[idx_y, 0], self.yy[idx_y+1, 0]
            
            # 获取四个点的值
            z00 = beam_profile[idx_y, idx_x]
            z01 = beam_profile[idx_y, idx_x+1]
            z10 = beam_profile[idx_y+1, idx_x]
            z11 = beam_profile[idx_y+1, idx_x+1]
            
            # 双线性插值公式
            x_weight = (x_val - x0) / (x1 - x0)
            y_weight = (y_val - y0) / (y1 - y0)
            
            interp_val = (
                z00 * (1 - x_weight) * (1 - y_weight) +
                z01 * x_weight * (1 - y_weight) +
                z10 * (1 - x_weight) * y_weight +
                z11 * x_weight * y_weight
            )
            return interp_val
        ...
        for i in range(grid_x.shape[0]):
            for j in range(grid_x.shape[1]):
                etch_rate_matrix[i, j] = interpolate_beam(grid_x[i, j], grid_y[i, j])
        
        # 转换为实际刻蚀率（单位：nm/s）
        # 实际中需要通过实验校准系数将相对强度转换为刻蚀率
        # 这里使用假设值：最大刻蚀率为10.0 nm/s
        max_rate = 10.0  # nm/s - 代表最大刻蚀能力
        max_beam = np.max(beam_profile)
        actual_etch_rate = etch_rate_matrix * (max_rate / max_beam)
        ...
        header = (
            ...
            f"最大刻蚀率: {max_rate:.2f} nm/s\n"
            f"适用于卷积运算（文件格式为CSV，无表头）"
        )
        
        # 保存数据（不带任何坐标信息）
        np.savetxt(output_path, actual_etch_rate, delimiter=",", header=header, comments='#')
        print(f"低分辨率(31×31)离子束刻蚀能力分布已保存至: {output_path}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 新增加：输出31×31低分辨率刻蚀能力分布
        csv_path_lowres = reconstructor.export_low_res_beam_csv("ion_beam_etch_rate_31x31.csv")
