...
class BeamEfficiencyOptimizer:
    ...
    def create_region_masks(self, beam_matrix):
        ...
        # 中心区域掩膜（FWHM+缓冲）
        self.center_mask = (r_center <= self.center_threshold) & (r_center <= self.opt_radius)
        ...
        # 更新日志信息
        self.log(f"分区优化: 中心区域(0-{self.center_threshold:.1f}mm) {center_points}点, "
             f"边缘区域({self.center_threshold:.1f}-{self.opt_radius}mm) {edge_points}点")
