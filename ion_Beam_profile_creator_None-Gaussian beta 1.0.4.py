...
class BeamEfficiencyOptimizer:
    ...
    def enforce_radial_constraints(self, beam_matrix):
        ...
        for angle in angles:
            ...
            # 4.2 确保下降速率先增后减（次要约束，只在违反时应用）
            if len(ray_values) < 5:  # 点数不足跳过
                continue
                
            # 计算下降速率（导数的绝对值）
            deriv = np.abs(np.diff(ray_values))
            
            # 计算二阶导数（速率的变化）
            sec_deriv = np.diff(deriv)
            
            # 查找拐点位置（二阶导数的符号变化点）
            sign_changes = np.where(np.diff(np.sign(sec_deriv)) != 0)[0]
            
            # 检查拐点数量
            if len(sign_changes) != 1:  # 要求恰好一个拐点
                self.log(f"方向 {np.rad2deg(angle):.0f}°: {len(sign_changes)} 个拐点，需要调整")
                
                # 创建理想速率曲线 (单峰形态)
                # 假设拐点位于路径的1/3处（经验值）
                peak_idx = int(len(deriv) * 0.33)
                
                # 创建理想化的目标下降速率
                # 在拐点前速率线性增加到峰值，之后线性减少
                target_rates = np.zeros(len(deriv))
                peak_rate = np.percentile(deriv, 80)  # 使用80%分位数作为峰值
                
                # 拐点前：递增
                if peak_idx > 0:
                    target_rates[:peak_idx] = np.linspace(0.2*peak_rate, peak_rate, peak_idx)
                
                # 拐点后：递减
                if peak_idx < len(target_rates) - 1:
                    target_rates[peak_idx:] = np.linspace(peak_rate, 0.1*peak_rate, len(target_rates)-peak_idx)
                
                # 构建目标值序列（从实际中心值开始）
                target_values = [ray_values[0]]
                for k in range(len(deriv)):
                    target_values.append(target_values[-1] - target_rates[k])
                
                # 混合目标和当前值（避免突变）
                for k in range(len(valid_x)):
                    # 跳过中心点和附近点（避免改变中心区域特征）
                    if ray_distances[k] < 3.0:  # 3mm内保持不变
                        continue
                    
                    # 应用混合（70%目标值 + 30%当前值）
                    new_val = 0.7 * target_values[k] + 0.3 * ray_values[k]
                    if abs(new_val - beam_matrix[valid_x[k], valid_y[k]]) > beam_matrix[valid_x[k], valid_y[k]] * 0.05:  # 变化>5%才应用
                        beam_matrix[valid_x[k], valid_y[k]] = max(0, new_val)
                        modified_points += 1
    ...
    def mutate_beam(self, beam_matrix, magnitude):
        ...
        for i in range(len(valid_indices[0])):
            ...
            mutation_prob = 0.45 if self.region_stage == "center" else 0.25
            ...
            if np.random.rand() < mutation_prob:
                ...
                dist = np.sqrt((self.grid[x] - center_x)**2 + (self.grid[y] - center_y)**2)
    ...
    def run_optimization(self, max_iterations=100, target_rel_error=10.0):
        ...
        for iteration in range(1, stage_iterations + 1):
            ...
            for _ in range(4):  # 4个变异候选
                candidates.append(self.mutate_beam(current_matrix, magnitude))
            ...
            for candidate in candidates:
                ...
                abs_error, rel_err_x, rel_err_y = self.calculate_error(sim_x, sim_y, region_mask=True)
        ...
        for iteration in range(1, stage_iterations + 1):
            ...
            # 4.4 记录评估结果
            if best_candidate is not candidates[0]:
                improvement = np.mean(self.history["abs_error"][-1]) - best_abs_error
        ...
        self.log(f"优化完成! 总迭代次数: {iteration}")
